...
(function () {
  ...
  App.Views.Gradetype = Backbone.View.extend({
    ...
    goweekindex: function(){
      var year,mongth,day;
      if($('.week select').length==1){
        $.tips({
          content: '请先联系管理员配置学期信息',
          stayTime: 2000,
          type: "warn"
        });
        return;
        ...
        success: function success(response) {
          ...
          if (result.code == '0') {
            ...
            for(var i=0,ilen=result.list.length;i<ilen;i++){
              ...
              var roleIdsArry=(result.list[i].roleIds+App.g.adminRoleIds).split(',');
              if(roleIdsArry.indexOf(''+App.g.gradesetting.get('roleId')+'')>=0){
                num++;
                str+='<button data-checkid="'+ result.list[i].checkId +'" data-tableid="'+ result.list[i].tableId +'" value="'+ result.list[i].title+'" >'+ result.list[i].title +'</button>';
              }
            }
            str+='</div></div><button class="cancel">取消</button>';
            menu.append(str);
            if(num>0){
              _selfthis.refreshWraper(num);
            }else{
              $.tips({
                content: '目前没有抽查项',
                stayTime: 2000,
    ...
    chooseButton: function(event){
      ...
      if(_this.data('checkid')){
        ...
        App.g.checkTableid=_this.data('tableid');
    ...
    showCheckArea: function(){
      ...
      $.ajax({
        ...
        success: function success(response) {
          var result = JSON.parse(response);
          if (result.code == '0') {
            if(result.list.cmpusList.length==0){
              $.tips({
            ...
            str+='<h4>请选择抽查生活区</h4>';
            str+='<div id="wrapper"><div id="scroller">';
            for(var i=0,ilen=result.list.cmpusList.length;i<ilen;i++){
              for(var j=0,jlen=result.list.cmpusList[i].liveAreaList.length;j<jlen;j++){
                App.g.checkAreaList.push(result.list.cmpusList[i].liveAreaList[i]);
                num++;
                str+='<button data-areaid="'+ result.list.cmpusList[i].liveAreaList[j].liveAreaId +'" value="'+ result.list.cmpusList[i].liveAreaList[j].title+'" >'+ result.list.cmpusList[i].liveAreaList[j].title +'</button>';
              }
            }
            str+='</div></div><button class="cancel">取消</button>';
            menu.append(str);
    ...
    showCheckFlat: function(){
      ...
      str+='<h4>请选择抽查楼栋</h4>';
      str+='<div id="wrapper"><div id="scroller">';
      var flatlist=App.g.checkAreaList.where({liveAreaId:App.g.liveAreaId})[0].attributes.flatList;
      for(var i=0;i<flatlist.length;i++){
        str+='<button data-flatid="'+ flatlist[i].flatId +'" value="'+ flatlist[i].title+'" >'+ flatlist[i].title +'</button>';
      }
      str+='</div></div><button class="cancel">取消</button>';
    ...
    refreshWraper: function(len){
      ...
      setTimeout(function () {
        ...
        if(len>6){
          ...
          _selfthis.myScroll = new fz.Scroll('#wrapper', {scrollY: true });
        }
