...
(function () {
  ...
  App.Views.Login = Backbone.View.extend({
    ...
    getRoleIdsStr: function(){
      var _selfthis=this;
      $.ajax({
        url: App.URL.getRoleIdsList + '?type=2&token='+ App.g.token,
        type: 'GET',
        dataType: 'JSON',
        success: function success(response) {
          var result = JSON.parse(response);
          if (result.code == '0') {
            ...
            for(var i=0,ilen=result.data.list.length;i<ilen;i++){
                App.g.adminRoleIds+=','+result.data.list[i].roleId;
            }
